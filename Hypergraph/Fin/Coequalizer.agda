...
----------------------------------------------------------------------------------------------------
-- Here we prove that coequalizer has a right inverse.
...
coequalizer1-helper-universal-∃ : ∀ {a b n} {C : Setoid a b} (f g : Fin (suc n)) (f<g : toℕ f < toℕ g) 
                          (d : Fin (suc n) → Setoid.Carrier C) →
                          d f ⟨ Setoid._≈_ C ⟩ d g → 
                          Σ (Fin n → Setoid.Carrier C) 
                            λ h → ∀ x → h (coequalizer1-helper f g f<g x) ⟨ Setoid._≈_ C ⟩ d x
coequalizer1-helper-universal-∃ {n = n} {C = C} f g f<g d d-eq = 
  h , he=d
  where
    ...
    he=d : ∀ x → h (coequalizer1-helper f g f<g x) ⟨ Setoid._≈_ C ⟩ d x
...
coequalizer1-universal-∃ : ∀ {a b n} {C : Setoid a b} (f g : Fin n) (d : Fin n → Setoid.Carrier C) →
                           d f ⟨ Setoid._≈_ C ⟩ d g → 
                           Σ (Fin (proj₁ (coequalizer1 f g)) → Setoid.Carrier C) 
                             λ h → ∀ x → h (proj₂ (coequalizer1 f g) x) ⟨ Setoid._≈_ C ⟩ d x
coequalizer1-universal-∃ {n = zero} () () d d-eq
coequalizer1-universal-∃ {n = suc n} {C = C} f g d d-eq with cmp (toℕ f) (toℕ g)
... | tri≈ ¬a b ¬c = d , (λ x → Setoid.refl C)
... | tri< a ¬b ¬c = coequalizer1-helper-universal-∃ {C = C} f g a d d-eq
... | tri> ¬a ¬b c = coequalizer1-helper-universal-∃ {C = C} g f c d (Setoid.sym C d-eq)

-- The existence of a morphism which in composition with a coequalizer
-- produces the given morphism similar to the coequalizer.
...
coequalizer-universal-∃ {m = zero} {n = n} {C = C} f g d d-eq = d , (λ x → Setoid.refl C)
coequalizer-universal-∃ {m = suc m'} {n = n} {C = C} f g d d-eq 
  with coequalizer {m'} (f ∘ inject₁) (g ∘ inject₁)
     | coequalizer1 (q (f (fromℕ m'))) (q (g (fromℕ m')))
     | coequalizer-universal-∃ {m = m'} {C = C} (f ∘ inject₁) (g ∘ inject₁) d (d-eq ∘ inject₁)
     | coequalizer1-universal-∃ {C = C} 
         (q (f (fromℕ m'))) (q (g (fromℕ m'))) h
         hqf=hqg
  where
    open Setoid C renaming (_≈_ to _∼_)
    open Relation.Binary.EqReasoning C
    q = proj₂ (coequalizer {m'} (f ∘ inject₁) (g ∘ inject₁))
    h = proj₁ (coequalizer-universal-∃ {m = m'} {C = C} (f ∘ inject₁) (g ∘ inject₁) d (d-eq ∘ inject₁))
    he=d = proj₂ (coequalizer-universal-∃ {m = m'} {C = C} (f ∘ inject₁) (g ∘ inject₁) d (d-eq ∘ inject₁))
...
-- inverse of the coequalizer.

coequalizer-universal-! : ∀ {a b m n} {C : Setoid a b} (f g : Fin m → Fin n) (d : Fin n → Setoid.Carrier C) →
                          (∀ x → d (f x) ⟨ Setoid._≈_ C ⟩ d (g x)) → 
                          (h : Fin (proj₁ (coequalizer f g)) → Setoid.Carrier C) →
                          (∀ x → h (proj₂ (coequalizer f g) x) ⟨ Setoid._≈_ C ⟩ d x) →
                          (∀ x → h x ⟨ Setoid._≈_ C ⟩ (d ∘ proj₁ (coequalizer-rinv f g)) x)
coequalizer-universal-! {C = C} f g d df=dg h he=d x = 
  begin
    h x
  ≡⟨ ≡-cong h (≡-sym (proj₂ (coequalizer-rinv f g) x)) ⟩
    h ((proj₂ (coequalizer f g)) ((proj₁ (coequalizer-rinv f g)) x))
  ≈⟨ he=d _ ⟩
    d ((proj₁ (coequalizer-rinv f g)) x)
  ∎
  where
    open Relation.Binary.EqReasoning C